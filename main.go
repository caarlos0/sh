...
func main() {
	...
	addr := ":" + os.Getenv("PORT")
	log.Infof("started up on %s", addr)
	if err := http.ListenAndServe(addr, httplog.New(r)); err != nil {
		log.Fatal(err.Error())
	}
}
...
func generateShell(ctx gocontext.Context, owner, repo, version string) (string, error) {
	...
	for _, f := range []string{
		...
		sh, err := tryFile(ctx, owner, repo, tag, f)
		if err != nil {
			return "", err
		}
		if sh == "" {
			// we didnt get an error and sh is empty, wrong file, try next
			continue
		}
		return sh, nil
	}
...
func tryFile(ctx gocontext.Context, owner, repo, tag, f string) (string, error) {
	...
	grctx := context.Wrap(ctx, proj)
	pipe := defaults.Pipe{}
	if err := pipe.Run(grctx); err != nil {
	...
	cfg := struct {
		...
		Project: grctx.Config,
		Version: tag,
	}
	err = template.Must(template.New("shell").Parse(shell)).Execute(&out, cfg)
	return out.String(), errors.Wrap(err, "failed to generate shell")
}

var shell = `#!/bin/sh
set -e
#  Code generated by goreleaser/sh. DO NOT EDIT.
#
usage() {
  this=$1
  cat <<EOF
...
Generated by goreleaser/sh
 https://github.com/goreleaser/sh
EOF
  exit 2
}
parse_args() {
